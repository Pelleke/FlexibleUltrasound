...
import keras
import tensorflow 
# from AugVal_reg import *
import scipy.io as sio
from AugVal_reg import AugVal_reg


class DataGenVal_reg(tensorflow.compat.v2.keras.utils.Sequence):  # tf.compat.v1.keras.utils.Sequence
    ...
    def __getitem__(self, idx):
        ...
        for i, ID in enumerate(batch):
            ...
            for j in range(self.dim[2]):  # Extract input image in dim(128,128,99)
                ...
                # selects all rows and shifts with
                tmp_vol_in[:, :, j] = raw_vol_in[:, j*self.dim[0]:self.dim[1]+j*self.dim[1]]

            # Call the data augmentation function 
            Vols = AugVal_reg(tmp_vol_in, tmp_vol_out, self.num_out, self.minmax)
